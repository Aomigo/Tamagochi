...
const informationTextArtefacts = document.querySelector('.text-info')

//Les animation empêche l'animation d'idle de fonctionner
let deathAnimation = false;
let crouchAnimation = false;
let animation = false;
let runAnimation = false
//Depleted... Sert a ajouter des dégâts au pv pour chaque barre vide
let depletedMental = false
let depletedExhaustion = false
let depletedHapiness = false
let depletedHunger = false
//Nombre de dégâts en fonction de depleted... true
let dependsOnHealth = 1
//permet de savoir quand un stack est trop grand
let tooMuchStacks = false
//change le fait de trouver des ingredients en trouver un artefact
let foundArtefact = false
//Le pourcentage de chaque barre
let TotalHealth = 100
let TotalMental = 100
let TotalExhaustion = 100
let TotalHapiness = 100
let TotalHunger = 100
//Variable globale car change les frames de l'idle
let frameTransitorWarrior = 0
//Change le dialogue
let dialogue = 0
//Utilisé juste au début pour stack
let bagpack = []
/*what to add : 
...
let phrase = ""
let pushedPhrase = []
//Création du dialogue au début du jeu
timeout()
function timeout() {
...
//Pour continuer sur le dialogue
...
//Verifiaction des dégâts en fonction de chaque bar vide
function MoreDecay() {
    ...
    //On check toute les x secondes pour éviter un overflow
    setTimeout(() => {
        MoreDecay()
    }, 3000)
}
...
//Peut être ajouter + de personnage. L'élement aimé et detesté est aléatoire et ne peux pas être le même
...
//Ajout de x ingrédients en début de run
for (let i = 0; i < 4; i++) {
...
//Animation de transition pendant l'aventure
function Running() {
    runAnimation = true
    let runPattern = 0
    runWarrior()
...
//Baisse progressive des barres, tout est modifiable ici
function Decay() {
    timeoutHealth()
    timeoutMental()
    timeoutExhaustion()
